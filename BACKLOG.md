...
| 014 | Feature | 关键信息确认或生成时，实现“朱红印章盖下”的视觉交互反馈 | Done | 1.2 |
...
| 213 | Bug/UI | 修复官职卡片样式偏移、扇形动画消失以及展开/定位不顺滑的问题：修正 CSS 选择器、优化 `max-height` 过渡逻辑并移除平移动画延迟 | Done | 1.2 / 2.2 |
| 214 | UI/UX | 增加镜头锁定机制：在动画期间利用 `requestAnimationFrame` 连续追踪并在反向施加位移，保持节点在横向展开子节点时位置静止 | Done | 1.2 / 2.2 |
| 215 | Bug | 修复 SVG 树状连线在重排动画末端脱节的问题：将连线高频重绘生命周期延长至 1100ms 以完全覆盖新的布局过渡时序 | Done | 1.2 / 2.2 |
| 216 | Bug | 修复节点展开时连线“瞬移/未平滑过渡”问题：提取真实 `ul` 的 CSS Overflow 物理裁剪蒙版边界，并将其作为包围盒参数注入到 SVG 热更重绘引擎中，实现连线的动态遮罩裁切动画 | Done | 1.2 / 2.2 |
| 217 | Bug | 修复节点横向展开存在“跃变”错觉：通过动态测量 `scrollWidth` 替代硬编码的 5000px 目标值，使得 CSS 的 800ms 黄金分割过渡能 100% 作用于真实的物理宽度，而非前 10% 提早跑完 | Done | 1.2 / 2.2 |
...
| 224 | Feature | 核心数据改造：实现“风流人物”属性的向上卷（Roll-up）继承机制，保证名人在具体底层官职及其各级所属机构上均能闭环显示 | Done | 3.2 |
| 225 | Feature | (Architecture) 确立文化类数据（`allusions`, `poetry`，如“典故连珠”）的归属边界准则：**语义就近原则**。此类数据不参与 Roll-up，而是紧接着其最初发生或形容的层级挂载。若是总结全机构风貌（如台阁生风）则放高层；若是单独官职（如江州司马）则放底层。 | Done | 3.2 |
