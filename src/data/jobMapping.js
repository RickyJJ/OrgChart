...
export const jobDictionary = {
    ...
    "产品经理": { title: "中书舍人", rank: "正五品上", desc: "掌侍进奏、参议表章、草拟诏旨" },
...
/**
 * 匹配现代职业到古代官职
 * @param {string} input - 用户输入的现代职业
 * @returns {{ title: string, rank: string, desc: string }}
 */
export function matchJob(input) {
    ...
    if (jobDictionary[normalized]) {
        return jobDictionary[normalized];
    }
    ...
    for (const entry of fuzzyKeywords) {
        if (entry.keywords.some(kw => lowerInput.includes(kw.toLowerCase()))) {
            return jobDictionary[entry.match];
        }
    }
